...
# GUIAS DE ONDA
#-------------------------------------------------------------------------------
WaveGuides<-function(kx,ky,kz,xo,yo,zo,lmax,mode=0){
   gama<-sqrt(kx^2+ky^2)
   k<-sqrt(gama^2+kz^2)
   LMAX=lmax*(lmax+2)+1
   #----------------------------------------
   ...
   if(mode==3){
      ...
      Gf<-CylindricalWG(kx,ky,kz,xo,yo,zo,lmax,s=sign(R),TM=FALSE)
      GTE<-B*Gf
      GTM<-A*Gf
      return(data.frame(GTE,GTM))
   }
   if(mode==4){
      ...
      Gf<-CylindricalWG(kx,ky,kz,xo,yo,zo,lmax,s=sign(R),TM=TRUE)
      GTE<- A*Gf
      GTM<--B*Gf
      return(data.frame(GTE,GTM))
   }
...
#WaveGuides(M,N,R,xo,yo,zo,lmax,mode=3)
#WaveGuides(M,N,R,xo,yo,zo,lmax,mode=4)
CylindricalWG<-function(m,n,r,x,y,z,lmax,s=1,TM=FALSE){
   #----------------------------------------
   # m,n -> n-esima raiz da m-esima funcao de Bessel
   # m -> coluna == ordem da funcao de Bessel
   # n -> linha  == raiz  da funcao de Bessel 
   #----------------------------------------
   ...
   if((m>5)||(m<0)){
      m<-0
      cat("m deve estar entre 0 e 5. m ajustado para 0")
   }
   #----------------------------------------
   if((n>6)||(m<1)){
      m<-1
      cat("n deve estar entre 1 e 6. n ajustado para 1")
   }
   #----------------------------------------
   if(TM){
      gama<-ZdJ[n,m+1]/r
      cat("TM: gamma =",gama,"M =",m,"\n")
   }else{
      gama<-ZJ[n,m+1]/r
      cat("TE: gamma =",gama,"M =",m,"\n")
   }
   #----------------------------------------
   LMAX=lmax*(lmax+2)+1
   g<-rep(0,LMAX)
...
# PSI(k,r) LENZ
#-------------------------------------------------------------------------------
psi.lenz<-function(gama,kz,x,y,z,m,s=1){
   rho<-sqrt(x^2+y^2)
   if((x==0)&&(y==0)){
      cph<-1
      sph<-0
